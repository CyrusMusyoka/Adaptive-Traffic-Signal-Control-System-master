...
# open-source library for deep learning application ,manages all aspect of a machine learning system
import cv2
# cv2 has function to read video which is videocapture().Real-time object detection , image segmentation
# deep neural network ,detect all faces
import numpy as np
# used for arrays as they are faster and more compact than python lists and as array uses less memory , numphy
# uses more less memory to store data
import time
import dlib
# break the task of identifying the face into thousands of smaller, face detection
import tensorflow.compat.v1 as tf
# for data automation, model tracking , performance monitoring and model retraining
import os
# provides functions for creating and removing directory ,fetching contents ,changing
# and identifying current directory
import threading
...
def countVehicles(param):
    ...
    # Shows original video as output and not the 416x416 image that is used as
    # yolov3 input (NOTE: Detection still happens with 416x416 img size but the output is
    # displayed in original video size if this parameter is True)
